...
<div class="calc-selection">
    ...
    <div class="fom-slider-container">
        ...
        <input type="range" id="fomSlider" class="fom-slider" min="-9" max="0" step="0.5" value="-3" oninput="updateFomDisplay(); updateCalculation()">
...
<div class="survey-container">
    ...
    <div class="input-section">
        ...
        <div class="jitter-box" id="jitterBox">
            <div class="jitter-title">Jitter requirement assuming SNR<sub>Jitter</sub> = SNDR + 3dB</div>
            <div class="evaluation-field" id="jitterField1" style="border-color: #6f42c1; color: #6f42c1; font-size: 0.5em; margin-top: 0;">Input is a single tone at FsNyq/2: Not calculated</div>
            <div class="evaluation-field" id="jitterField2" style="border-color: #6f42c1; color: #6f42c1; font-size: 0.5em;">Input is uniform spectrum from 0 to FsNyq/2: Not calculated</div>
        </div>
    </div>
</div>
...
function updateCalculation() {
    ...
    function clearJitter() {
        document.getElementById('jitterField1').textContent = 'Input is a single tone at FsNyq/2: Not calculated';
        document.getElementById('jitterField2').textContent = 'Input is uniform spectrum from 0 to FsNyq/2: Not calculated';
    }

    function updateJitterDisplay(sndr, fsNyq) {
        const jitter1 = calculateJitter1(fsNyq, sndr);
        const jitter2 = calculateJitter2(fsNyq, sndr);
        document.getElementById('jitterField1').textContent = `Input is a single tone at FsNyq/2: ${formatEngineering(jitter1, 's')}`;
        document.getElementById('jitterField2').textContent = `Input is uniform spectrum from 0 to FsNyq/2: ${formatEngineering(jitter2, 's')}`;
    }
    ...
    if (mode === 'power') {
        ...
        updateJitterDisplay(sndr, fsNyq);
    } else if (mode === 'sndr') {
        ...
        updateJitterDisplay(result, fsNyq);
    } else { // fs mode
        ...
        updateJitterDisplay(sndr, result);
    }
}
...
// Jitter functions
// - jitter1: input is a single tone at FsNyq/2
const calculateJitter1 = (fsNyq, sndr) => Math.pow(2 * 3.14159 * fsNyq / 2 * Math.pow(10, (sndr+3)/20 ), -1)
// - jitter2: input is a uniform spectrum from 0 to FsNyq/2
const calculateJitter2 = (fsNyq, sndr) => Math.pow(3, 0.5) * calculateJitter1(fsNyq, sndr);
</script>